...
func (s *Server) handleSMSRead(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil || index < 0 { http.Error(w, "invalid SMS index", http.StatusBadRequest); return }
	svc, ok := s.smsService(w)
	if !ok { return }
...
func (s *Server) handleSMSSend(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if strings.Contains(msg, "required") || strings.Contains(msg, "invalid") || strings.Contains(msg, "empty") {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
...
func (s *Server) handleSMSDelete(w http.ResponseWriter, r *http.Request) {
	...
	svc, ok := s.smsService(w)
	if !ok { return }
	if err := svc.DeleteSMS(index); err != nil { writeUsecaseError(w, err); return }
	w.Header().Set("Content-Type", "application/json")
